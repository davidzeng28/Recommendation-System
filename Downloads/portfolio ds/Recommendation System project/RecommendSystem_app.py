...
def getdata():
    games_df = pd.read_csv('dataset/Games_dataset.csv', index_col=0)
    similarity_df = pd.read_csv('dataset/sim_matrix.csv', index_col=0)
    return games_df, similarity_df

games_df, similarity_df = getdata()[0], getdata()[1]

# Sidebar
st.sidebar.markdown('__ Nintendo Switch Game Recommendation(Choosing one game to find similar games)')
...
if selected_game:
    ...
    # DF query
    matches = similarity_df[selected_game].sort_values()[1:6] ## setting recommend 5 games
